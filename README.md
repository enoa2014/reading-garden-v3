...
- 已实现：打开项目、书架管理、新建书、`rgbook` 导入导出、`rgsite` 全量/子集发布包导出（subset 支持 balanced/minimal + missingAssets/MISSING-ASSETS.txt 分组+分类）
- 已实现：`rgbook` 导入失败诊断报告下载（完整/脱敏/自定义 + 最近模板复用/清空/导入导出/merge导入）、编辑器回归脚本
