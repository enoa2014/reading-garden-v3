...
- [x] Live Preview 自动刷新策略支持导入/导出（`replace/merge`，可选覆盖默认值）
- [x] 支持组合策略包导入/导出（会话快照策略 + 自动刷新策略）
- [x] 组合策略包导入兼容旧单策略文件（recovery/auto-refresh）
...
- [x] 支持手动清理会话快照
- [x] 会话快照按项目优先恢复
- [x] 会话快照支持最近 5 条历史恢复（按时间点选择）
- [x] 会话快照支持删除选中历史记录
- [x] 会话快照历史支持按时间自动清理（默认 30 天）
- [x] 会话快照自动清理阈值支持 UI 配置（关闭/7/30/90/180 天）
- [x] 会话快照清理阈值支持按项目独立配置
- [x] 会话快照策略支持一键恢复全局默认（当前项目）
- [x] 会话快照策略显示当前来源（项目覆盖/全局默认）
- [x] 会话快照策略支持导入/导出（跨设备复制）
- [x] 会话快照策略导入支持 `replace/merge` 模式（replace 覆盖 / merge 合并项目策略）
- [x] 策略导入支持 merge 可选覆盖默认值（默认关闭）
- [x] 新建书支持模板级别（minimal/standard/teaching/custom）
- [x] 新建书模板预设支持自定义保存与导入导出
...
3. [已完成 2026-02-12] EdgeOne 预检自测已覆盖“最小样例 + 真实资产样例 + 失败断言（checksum mismatch/unsafe path）”。
...
| 会话快照提供手动清理入口 | 支持主动重置恢复上下文，避免误恢复干扰当前操作 |
| 会话恢复改为按项目优先匹配并保留 latest 兜底 | 降低跨项目误恢复概率，同时保持兼容旧快照行为 |
| 会话快照历史先固定保留最近 5 条并提供按时间点恢复 | 先满足断电恢复与误操作回退，再按用户反馈决定是否开放可配置上限 |
| 会话快照历史支持“删除选中项”且不影响整库清理入口 | 提供细粒度治理能力，降低误删全部历史的风险 |
| 会话快照历史默认启用“30 天自动清理” | 控制本地 IndexedDB 累积体积，避免长期离线使用产生无界增长 |
| 会话快照清理阈值支持 UI 配置（关闭/7/30/90/180）并写入 localStorage | 允许不同设备性能与使用习惯下灵活调整，而无需改代码 |
| 会话快照清理阈值支持按项目独立配置（项目覆盖全局默认） | 同一设备可能维护多个项目，允许按项目设定不同快照保留策略 |
| 会话快照策略支持一键恢复全局默认（当前项目） | 减少手动回填默认值操作成本，降低项目级策略残留风险 |
| 会话快照策略在 UI 显示“来源标签（项目覆盖/全局默认）” | 降低用户对当前生效策略的认知负担，减少误操作与排查成本 |
| 会话快照策略支持导入/导出（单文件） | 先满足跨设备迁移诉求，后续再评估细粒度导入模式 |
| 会话快照策略导入新增 `replace/merge` 模式，`merge` 仅合并项目映射并保留本地全局默认 | 降低跨设备增量迁移对现有默认策略的冲击，避免误改全局保留周期 |
| 策略导入新增“merge 可选覆盖默认值”开关，默认关闭 | 保持既有 merge 行为兼容，同时允许按需执行全量迁移 |
...
| 用户确认：新建书自定义模板预设按项目隔离暂不做 | 当前全局存储已满足阶段需求，优先推进发布链路与迁移稳定性 |
...
| Live Preview 自动刷新策略导入支持 `replace/merge`，并可选覆盖默认值 | 兼顾安全增量迁移与全量同步场景 |
| 新增组合策略包导入/导出（恢复策略 + 自动刷新策略） | 降低迁移步骤复杂度，支持一次文件完成多策略同步 |
| 组合策略包导入兼容旧单策略文件格式 | 降低升级成本，避免用户误选文件导致迁移失败 |
| 用户确认：组合策略包密码保护/敏感字段脱敏暂不做 | 当前离线受控场景优先透明可调试，后续按合规需求再引入加密 |
